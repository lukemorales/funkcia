...
describe('OptionAsync', () => {
  describe('constructors', () => {
    describe('resource', () => {
      ...
      it('wraps a resource and handles errors safely', async () => {
        const db = OptionAsync.resource(database);
    ...
    describe('fromOption', () => {
      it('converts an Option to an OptionAsync', async () => {
        const task = OptionAsync.fromOption(Option.some(10));
      ...
      it('converts a None Option to an OptionAsync None', async () => {
        const task = OptionAsync.fromOption(Option.none<number>());
        expectTypeOf(task).toEqualTypeOf<OptionAsync<number>>();
    ...
    describe('fromResult', () => {
      it('converts an Ok Result to an OptionAsync Some', async () => {
        const task = OptionAsync.fromResult(Result.ok(10));
        expectTypeOf(task).toEqualTypeOf<OptionAsync<number>>();
      ...
      it('converts an Error Result to an OptionAsync None', async () => {
        const task = OptionAsync.fromResult(
          Result.error(new Error('computation failed')),
        );
        expectTypeOf(task).toEqualTypeOf<OptionAsync<never>>();
    ...
    describe('fromResultAsync', () => {
      it('converts an Ok ResultAsync to an OptionAsync Some', async () => {
        const task = OptionAsync.fromResultAsync(ResultAsync.ok(10));
        expectTypeOf(task).toEqualTypeOf<OptionAsync<number>>();
      ...
      it('converts an Error ResultAsync to an OptionAsync None', async () => {
        const task = OptionAsync.fromResultAsync(
    ...
    describe('Do', () => {
      it('creates an `OptionAsync` with an empty object branded with the DoNotation type', async () => {
        ...
        expectTypeOf(task).toEqualTypeOf<OptionAsync<DoNotation>>();
    ...
    describe('fn', () => {
      it('creates a new function that returns an OptionAsync from a Promise that resolves to an Option', async () => {
        const task = OptionAsync.fn((greeting: string) =>
          Promise.resolve(Option.of(greeting)),
        );
        expectTypeOf(task).toEqualTypeOf<
          (greeting: string) => OptionAsync<string>
        >();

        const option = await task('hello world');
      ...
      it('returns a function that safely evaluates the generator, returning the returned OptionAsync when all yields are `Some`', async () => {
        const execute = OptionAsync.fn(async function* (
          greeting: string,
          subject: string,
        ) {
  ...
  describe('instance', () => {
    describe('bindTo', () => {
      it('binds the current `OptionAsync` to a `do-notation`', async () => {
        ...
        expectTypeOf(task).toEqualTypeOf<
          OptionAsync<DoNotation<{ a: number }>>
        >();
    ...
    describe('unwrap', () => {
      ...
      it('rejects if the OptionAsync is a None', async () => {
        ...
        await expect(task.unwrap()).rejects.toThrow();
      });
    });
