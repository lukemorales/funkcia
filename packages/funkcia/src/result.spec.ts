  ...
  describe('constructors', () => {
    ...
    describe('Do', () => {
      it('creates an `Result` with an empty object branded with the DoNotation type', () => {
        ...
        expectTypeOf(result).toEqualTypeOf<Result<DoNotation, never>>();
      ...
      it('creates an Error Result when the function throws', () => {
        {
          ...
          expectTypeOf(result).toEqualTypeOf<
            Result<never, UnhandledException>
          >();

          expect(result.isError()).toBeTrue();
          expect(result.unwrapError()).toEqual(
            new UnhandledException(String(new Error('computation failed'))),
          );
        }
    ...
    describe('fun', () => {
      class UnsetSetting extends TaggedError('UnsetSetting') {}

      class DisabledSetting extends TaggedError('DisabledSetting') {}
      ...
      it('returns a function with improved inference without changing behavior', () => {
        ...
        const wrapped = Result.fn(hasEnabledSetting);
      ...
      it('returns an async function with improved inference without changing behavior', async () => {
        function* hasEnabledSetting(enabled: boolean | null) {
          switch (enabled) {
            case true:
              return Result.ok(true as const);
            case false:
              return yield* Result.error(new DisabledSetting());
            default:
              return yield* Result.error(new UnsetSetting());
          }
        }
        ...
        expectTypeOf(hasEnabledSetting).toEqualTypeOf<
          (
            ...
            DisabledSetting | UnsetSetting,
            Result<true, never>,
            undefined
          >
        >();

        const wrappedFn = Result.fn(hasEnabledSetting);
    ...
    describe('lift', () => {
      class InvalidDivisor extends TaggedError('InvalidDivisor') {}
    ...
    describe('partition', () => {
      it('returns a tuple of arrays containing only the values inside `Ok` on the first position and `Error` on the second position', () => {
        const output = Result.partition([
          Result.ok(1),
        ...
        const [values, errors] = output;

        expect(values).toEqual([1, 3]);
        expect(errors).toEqual(['Failed computation']);
      });
    });
  ...
  describe('instance', () => {
    describe('bindTo', () => {
      it('binds the current `Result` to a `do-notation`', () => {
        ...
        expectTypeOf(option).toEqualTypeOf<
          Result<DoNotation<{ a: number }>, never>
        >();
    ...
    describe('unwrapError', () => {
      ...
      it('throws an Error when Result is an Ok', () => {
        ...
        expect(() => result.unwrapError()).toThrow(Panic);
      });
    });
