...
export function last<A>(self: readonly A[]): Option<A> {
  return Option.of(self)
    .filter(isNonEmpty)
    .map((_self) => _self[_self.length - 1] as A);
}
...
export function init<A>(self: readonly A[]): Option<NonEmptyArray<A>> {
  ...
  return Option.of(self)
    .filter(isNonEmpty)
    .map((_self) => _self.slice(0, self.length - offset) as any);
}
...
export function at<A>(index: number): (self: readonly A[]) => Option<A> {
  return (self) => {
    ...
    const item = index < 0 ? self[size + index] : self[index];

    return Option.fromNullable(item) as never;
  };
}
...
export function find(
  ...
  return (self) => Option.fromNullable(self.find(predicate));
}
...
export function findIndex<A>(
  ...
  return (self) =>
    Option.of(self.findIndex(predicate)).filter((index) => index >= 0);
}
...
export function filterMap<A, B>(
  ...
  return (self) => {
    ...
    for (let index = 0; index < self.length; index++) {
      console.log(index, at(index)(self).unwrap());
      Option.of(self)
        .flatMap(at(index))
        .flatMap((item) => callback(item, index, self))
        .match(
          () => null,
